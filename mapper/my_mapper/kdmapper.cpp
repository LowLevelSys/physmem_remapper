...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool remove_from_system_page_tables ,bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, bool PassSizeAsSecondParam,mapCallback callback, NTSTATUS* exitCode) {
	...
	if (mode == AllocationMode::AllocateMdl) {
		...
		kernel_image_base = intel_driver::AllocContiguousMemory(iqvw64e_device_handle, image_size);
	}
	...
	do {
		...
		NTSTATUS status = 0;
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point, PassAllocationAddressAsFirstParam ? realBase : 0, PassSizeAsSecondParam ? image_size : 0)) {
		...
		if (remove_from_system_page_tables) {

			// Get the physical image base if we want to remove it from physmem before we free it
			if (!intel_driver::GetPhysicalAddress(iqvw64e_device_handle, kernel_image_base, &physical_image_base)) {
				Log("[-] Failed to get physical image base");
				return false;
			}
		}
		...
		if (free) {
			...
			else if (mode == AllocationMode::AllocateIndependentPages)
			{
				...
				free_status = intel_driver::FreeContiguousMemory(iqvw64e_device_handle, realBase);
			}
			else {
		...
		if (remove_from_system_page_tables) {
			...
			LARGE_INTEGER lphys_base = { 0 };

			size.QuadPart = image_size;
			lphys_base.QuadPart = physical_image_base;

			if (!intel_driver::RemovePhysicalMemory(iqvw64e_device_handle, &lphys_base, &size)) {
				Log(L"[-] Failed to remove physical memory!" << std::endl);
			}

			Log(L"[+] Memory has removed from system page tables" << std::endl);
		}
	...
	else if (mode == AllocationMode::AllocateIndependentPages)
	{
		...
		free_status = intel_driver::FreeContiguousMemory(iqvw64e_device_handle, kernel_image_base);
	}
	else {
