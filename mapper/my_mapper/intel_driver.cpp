...
uint64_t intel_driver::AllocContiguousMemory(HANDLE iqvw64e_device_handle, uint64_t size) {
	if (!size)
		return 0;

	static uint64_t kernel_MmAllocateContiguousMemory = GetKernelModuleExport(iqvw64e_device_handle, intel_driver::ntoskrnlAddr, "MmAllocateContiguousMemory");
	...
	if (!CallKernelFunction(iqvw64e_device_handle, &allocated_pool, kernel_MmAllocateContiguousMemory, size, max_addr))
		return 0;

	return allocated_pool;
}

bool intel_driver::RemovePhysicalMemory(HANDLE iqvw64e_device_handle, PLARGE_INTEGER phys_base, PLARGE_INTEGER size) {
	if (!phys_base->QuadPart || !size->QuadPart)
		return false;

	static uint64_t kernel_MmRemovePhysicalMemory = GetKernelModuleExport(iqvw64e_device_handle, intel_driver::ntoskrnlAddr, "MmRemovePhysicalMemory");
	...
	NTSTATUS status;
	if (!CallKernelFunction(iqvw64e_device_handle, &status, kernel_MmRemovePhysicalMemory, phys_base, size))
		return false;

	return NT_SUCCESS(status);
}

bool intel_driver::FreeContiguousMemory(HANDLE iqvw64e_device_handle, uint64_t base) {
	if (!base)
		return false;

	static uint64_t kernel_MmFreeContiguousMemory = GetKernelModuleExport(iqvw64e_device_handle, intel_driver::ntoskrnlAddr, "MmFreeContiguousMemory");
	...
	return CallKernelFunction<void>(iqvw64e_device_handle, nullptr, kernel_MmFreeContiguousMemory, base);
}
